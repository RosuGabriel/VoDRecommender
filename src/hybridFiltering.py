...
from contentBasedFiltering import get_recommendations as content_based_recommender, get_user_profile
from collaborativeFiltering import train_recommender, import_model, get_recommendations as collaborative_recommender
import pandas as pd
...
class HybridRecommender:
    def __init__(self, data: MovieLensData=None, model=SVD(n_epochs=2000, lr_all=0.001, reg_all=0.1, n_factors=100), includeMyRatings: bool=True, freshModel: bool=False, modelName: str=None):
        self.data = data
    ...
    def initialize_recommender(self, userId: int, disagreementPenalty: float=0.5, collaborativeWeight: float=0.5, includeFeatures: bool=False, ratingsWeights: list=[1,1,1], titleWeight: int=2, genresWeights: list=[1], yearWeight: float=1.0):
        ...
        # Get user profile
        self.userProfile = get_user_profile(userId=self.userId, data=self.data, ratingsWeights=self.ratingsWeights, titleWeight=self.titleWeight, genresWeights=self.genresWeights, yearWeight=self.yearWeight)
        ...
        if includeFeatures:
            ...
            self.cbfRecommendations = content_based_recommender(data=self.data, userProfile=self.userProfile, recomandationsNum=-1)
        else:
            ...
            self.cbfRecommendations = content_based_recommender(data=self.data, userProfile=self.userProfile, recomandationsNum=-1)[['movieId', 'title', 'year', 'predictedRatingCBF']]

        # Create the hybrid recommendations
    ...
    def change_user(self, userId: int):
        self.userId = userId
        self.userProfile = get_user_profile(self.userId, self.data, ratingsWeights=self.ratingsWeights, titleWeight=self.titleWeight, genresWeights=self.genresWeights, yearWeight=self.yearWeight)
    ...
    def user_real_ratings(self):
        return self.data.mergedDf[self.data.mergedDf['userId'] == self.userId][['movieId', 'title', 'year', 'rating']].dropna()
